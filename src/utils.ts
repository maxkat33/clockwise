...
export const getCityData = (searchKey: string) : City | undefined => {
  return cityData.find((city) => city.searchKey.toLowerCase() === searchKey.toLowerCase());
}
...
export const getClosestCityData = (lat: number, lon: number): City => {
  ...
  for (const city of cityData) {
    ...
    if (dist < minDistance) {
      // If coords within 50km of city return 
      if (dist < 50) {
        return city
      }
      minDistance = dist;
...
export const getRandomCity = () => {
  ...
  console.log(cityData[randomNum].searchKey)
...
export const formatDigitalString = (timestamp: number, is24h: boolean): string => {
  ...
  let minutes = date.getHours().toString().padStart(2, "0")
  let seconds = date.getHours().toString().padStart(2, "0")
...
// export const formatLocationString = (searchKey: string) => {
//   const [city, country] = searchKey.split(', ')

//   const cityCapitalised = capitaliseString(city)
//   const countryCapitalised = country.split(' ').map((part)=> part[0].toUpperCase() + part.slice(1)).join(' ')

//   return cityCapitalised + countryCapitalised
...
export const formatTimezoneString = (utcOffset: number) => {
  ...
  let minutes = ((num % 1) * 60).toString().padStart(2, "0")
